...
const dashbots = [require('../src/dashbot')(process.env.DASHBOT_API_KEY_FACEBOOK,
  {debug:true, urlRoot: process.env.DASHBOT_URL_ROOT, redact: process.env.DASHBOT_REDACT}).facebook,
  require('../src/dashbot')(process.env.DASHBOT_API_KEY_FACEBOOK,
    {debug:true, urlRoot: process.env.DASHBOT_URL_ROOT2, redact: process.env.DASHBOT_REDACT}).facebook]
...
app.post(webHookPath, function(req, res) {
  _.each(dashbots, dashbot => {dashbot.logIncoming(req.body)});
    ...
    fetch(requestData.url + '?access_token=' + requestData.qs.access_token, {
      ...
      if(response.ok) {
        ...
        response.json().then(function(responseJson) {
          _.each(dashbots, dashbot => {dashbot.logOutgoing(requestData, responseJson)});
        })
      }
    });
  } else {
